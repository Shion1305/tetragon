...
// a TCP address runs that listener plus an in-pod plaintext unix socket
// at defaultUnixSocketPath, with --server-tls-* gating the TCP path.
//
...
func Serve(ctx context.Context, listenAddr string, srv *server.Server, logSrv *eventlog.Server, extraOpts ...grpc.ServerOption) error {
	...
	if err := serveOne(ctx, "unix", defaultUnixSocketPath, extraOpts, register); err != nil {
		return fmt.Errorf("starting unix gRPC listener: %w", err)
	}
